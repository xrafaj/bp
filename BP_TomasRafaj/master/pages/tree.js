...
const testTree = async () => {
        ...
        const Keccak256H = require('keccak256')
        const cards = ['0151', '0252', '0353', '0454', '0561', '0571']
        const leaves = ['0151', '0252', '0353', '0454', '0561', '0571'].map(x => Keccak256H(x))
        const tree = new MerkleTree(leaves, Keccak256H)
        const root = tree.getRoot().toString('hex')

        const leaf = Keccak256H('0151')
        const proof = tree.getProof(leaf)
        console.log(tree.verify(proof, leaf, root)) // true

        // const leaf2 = Keccak256H('b') //true
        const leaf2 = Keccak256H('0151')
        const leaf3 = Keccak256H('0454')
        const proof2 = tree.getProof(leaf3)
        console.log(tree.verify(proof2, leaf3, root)) // true

        let moves = []
        let cardsA = ['0151', '0252'];
        let cardsB = ['0353', '0454'];
        ...
        //  hit
        if(cardsA.push(cards[4])){
            moves.push('10')
        }
        
        if(cardsB.push(cards[5])){
            moves.push('11')
        }
    ...
    <div className="notes-container" style={{background: '#16191e'}}>
                <div className="container">
                    <div className="navbar-end">
                        ...
                        <button onClick={testTree} className='button' style={{background: '#e8b00b'}}>Test tree</button>
                    </div>
                </div>
